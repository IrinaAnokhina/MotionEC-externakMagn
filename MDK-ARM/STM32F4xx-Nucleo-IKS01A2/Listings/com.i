...
volatile uint8_t UartRxBuffer[(2 * 256)];
volatile uint32_t UsartBaudRate = 921600;

TUart_Engine UartEngine;

 
static volatile uint8_t UartTxBuffer[(2 * 256)];
...
int UART_ReceivedMSG(TMsg *Msg)
{
  ...
  {
    dma_counter = (uint16_t)(2 * 256) - (uint16_t)Get_DMA_Counter(&HdmaRx);
    ...
    {
      length = (uint16_t)(2 * 256) + dma_counter - UartEngine.StartOfMsg;
    }
    ...
    for (k = 0; k < length; k++)
    {
      ...
      if (data == (uint8_t)0xF0)
      {
        ...
        for (i = 0; i < k; i += inc)
        {
          ...
          j = (j + inc) % (uint16_t)(2 * 256);
          source++;
        }

        Msg->Len = source;
        j = (j + 1U) % (uint16_t)(2 * 256);  
        UartEngine.StartOfMsg = j;

        if (CHK_CheckAndRemove(Msg) != 0)  
        {
          return 1;
        }
      }
...
void USART_DMA_Configuration(void)
{
  ...
  do{ (&UartHandle)->hdmarx = &(HdmaRx); (HdmaRx). Parent = (&UartHandle); } while(0U);
}
...
void USARTConfig(void)
{
  ...
  gpio_init_struct.Pin       = ((uint16_t)0x0004);
  gpio_init_struct.Mode      = 0x00000002U;
  gpio_init_struct.Pull      = 0x00000000U;
  gpio_init_struct.Speed     = 0x00000002U;
  gpio_init_struct.Alternate = ((uint8_t)0x07);

  HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0000UL)), &gpio_init_struct);

   
  gpio_init_struct.Pin = ((uint16_t)0x0008);
  gpio_init_struct.Alternate = ((uint8_t)0x07);
  ...
  UartHandle.Instance        = ((USART_TypeDef *) (0x40000000UL + 0x4400UL));
  UartHandle.Init.BaudRate   = UsartBaudRate;
  UartHandle.Init.WordLength = 0x00000000U;
  UartHandle.Init.StopBits   = 0x00000000U;
  UartHandle.Init.Parity     = 0x00000000U;
  UartHandle.Init.HwFlowCtl  = 0x00000000U;
  UartHandle.Init.Mode       = ((uint32_t)((0x1UL << (3U)) | (0x1UL << (2U))));
  ...
  UartHandle.RxXferSize = (2 * 256);
  UartHandle.ErrorCode = (uint32_t)0x00000000U;

  
 
   
  (void)HAL_UART_Receive_DMA(&UartHandle, (uint8_t *)UartRxBuffer, (2 * 256));
}
