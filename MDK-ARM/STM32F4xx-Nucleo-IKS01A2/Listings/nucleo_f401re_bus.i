...
int32_t BSP_I2C1_Init(void)
{
  ...
  hbusi2c1.Instance  = ((I2C_TypeDef *) (0x40000000UL + 0x5400UL));
...
#line 128 "..\\Src\\nucleo_f401re_bus.c"
    ...
    else if (HAL_I2CEx_ConfigAnalogFilter(&hbusi2c1, 0x00000000U) != HAL_OK)
    {
      ret = -8;
    }
    else
...
int32_t BSP_I2C1_WriteReg(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t len)
{
  ...
  if (HAL_I2C_Mem_Write(&hbusi2c1, (uint8_t)DevAddr,
                        (uint16_t)Reg, 0x00000001U,
                        (uint8_t *)pData, len, 1000) == HAL_OK)
  {
    ret = 0;
  }
...
int32_t BSP_I2C1_Send(uint16_t DevAddr, uint8_t *pData, uint16_t len)
{
  ...
  if (HAL_I2C_Master_Transmit(&hbusi2c1, DevAddr, pData, len, 1000) == HAL_OK)
  {
    ret = len;
  }
...
int32_t BSP_I2C1_Recv(uint16_t DevAddr, uint8_t *pData, uint16_t len)
{
  ...
  if (HAL_I2C_Master_Receive(&hbusi2c1, DevAddr, pData, len, 1000) == HAL_OK)
  {
    ret = len;
  }
...
int32_t BSP_I2C1_SendRecv(uint16_t DevAddr, uint8_t *pTxdata, uint8_t *pRxdata, uint16_t len)
{
  ...
  if ((BSP_I2C1_Send(DevAddr, pTxdata, len) == len) &&       (BSP_I2C1_Recv(DevAddr, pRxdata, len) == len))
...
__weak HAL_StatusTypeDef MX_I2C1_Init(I2C_HandleTypeDef *hi2c)
{
  ...
  hi2c->Instance = ((I2C_TypeDef *) (0x40000000UL + 0x5400UL));
  hi2c->Init.ClockSpeed = 400000;
  hi2c->Init.DutyCycle = 0x00000000U;
  hi2c->Init.OwnAddress1 = 0;
  hi2c->Init.AddressingMode = 0x00004000U;
  hi2c->Init.DualAddressMode = 0x00000000U;
  hi2c->Init.OwnAddress2 = 0;
  hi2c->Init.GeneralCallMode = 0x00000000U;
  hi2c->Init.NoStretchMode = 0x00000000U;
...
if(i2cHandle->Instance==((I2C_TypeDef *) (0x40000000UL + 0x5C00UL)))
  {
